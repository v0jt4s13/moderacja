...
def render_via_openshot(project_dir: str, profile: Union[str, Dict[str, Any]] = "1080p") -> Dict[str, Any]:
    ...
    # print(f'cfg cfg cfg cfg cfg cfg cfg cfg cfg\n\t{cfg}\n')
...
def render_via_openshot_depr(project_dir: str, profile: Dict[str, Any]) -> Dict[str, Any]:
    """Convenience wrapper to render an existing json2video job via OpenShot.

    project_dir: path to the job directory (created by json2video)
    profile: dict with export settings (e.g., width, height, fps, filename)
    """
    job_config = {
    ...
    return start(job_config, str(Path(project_dir).parent))
